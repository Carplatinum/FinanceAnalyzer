...
@pytest.fixture
def sample_category_data() -> pd.DataFrame:
...
def test_expenses_by_category_report_mock_to_period(
    mock_to_period: MagicMock,
    sample_category_data: pd.DataFrame
) -> None:
    """Тест с мокированием метода to_period для проверки вызова."""
    mock_to_period.return_value = sample_category_data['Дата операции']
    start_date = datetime.datetime(2025, 4, 23)
    category = 'Продукты'
